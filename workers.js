...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  <head>
    ...
    <style>
      ...
      /* Definisikan keyframes untuk animasi pergerakan background */
...
/* Kelas untuk menerapkan animasi pada teks */
.animate-rainbow {
  /* 1. Sudut diatur menjadi 150 derajat dengan warna pastel pilihan Anda */
  background: linear-gradient(150deg, 
    ...
    #87CEFA, /* Biru Muda */
    #D8BFD8, /* Ungu Muda */
    #FFC0CB  /* Kembali ke warna awal agar loop mulus */
  );
  ...
  /* 2. Kecepatan animasi diperlambat menjadi 10 detik per siklus */
  ...
  <body class="bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-white bg-fixed transition-colors duration-300">
    ...
    >
      <div class="shrink-0">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 text-gray-800 dark:text-gray-200">
          <path
      ...
      <div>
        ...
        <p class="text-sm text-gray-700 dark:text-gray-300">Akun berhasil disalin</p>
      </div>
    </div>
    <!-- Select Country -->
    <div>
    ...
    <div class="ml-14">
        <!-- Header -->
        <header class="p-4 flex justify-between items-center text-lg">
        <h1 class="text-4xl font-bold">Welcome to <span class="animate-rainbow">Yumi P</span></h1>
            <div class="flex gap-6 items-center">
                ...
                <span class="text-gray-600 dark:text-gray-300">Time: <strong id="time-info-value" class="font-semibold">00:00:00</strong></span>
                <span class="text-gray-600 dark:text-gray-300">Your IP: <strong id="ip-value">127.0.0.1</strong></span>
                <span class="text-gray-600 dark:text-gray-300">ISP: <strong id="isp-value">Localhost</strong></span>
            </div>
        </header>

        <!-- Proxy Grid -->
        <main class="px-4 pb-24">
            <div id="proxy-grid-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                PLACEHOLDER_PROXY_GROUP
            </div>
        </main>

        <!-- Pagination -->
        <nav id="container-pagination" class="w-full fixed bottom-0 mb-6 flex justify-center items-center">
    ...
    <div id="container-window" class="hidden">
      <!-- Windows -->
      <!-- Informations -->
      <div class="fixed z-20 top-0 w-full h-full bg-gray-900/80 backdrop-blur-sm flex justify-center items-center">
        <p id="container-window-info" class="text-center w-full h-full top-1/4 absolute text-white"></p>
      </div>
      <!-- Output Format -->
      <div id="output-window" class="fixed z-30 top-0 right-0 w-full h-full flex justify-center items-center hidden">
        <div class="w-[75%] max-w-sm h-auto flex flex-col gap-2 p-4 text-center rounded-lg bg-gray-800 border border-gray-700">
            <div class="grid grid-cols-3 gap-2">
              <button onclick="copyToClipboardAsTarget('clash')" class="p-2 rounded-lg bg-amber-500 hover:bg-amber-600 flex justify-center items-center">Clash</button>
              <button onclick="copyToClipboardAsTarget('sfa')" class="p-2 rounded-lg bg-amber-500 hover:bg-amber-600 flex justify-center items-center">SFA</button>
              <button onclick="copyToClipboardAsTarget('bfr')" class="p-2 rounded-lg bg-amber-500 hover:bg-amber-600 flex justify-center items-center">BFR</button>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <button onclick="copyToClipboardAsTarget('v2ray')" class="p-2 rounded-lg bg-amber-500 hover:bg-amber-600 flex justify-center items-center">V2Ray/Xray</button>
              <button onclick="copyToClipboardAsRaw()" class="p-2 rounded-lg bg-amber-500 hover:bg-amber-600 flex justify-center items-center">Raw</button>
            </div>
            <div class="flex w-full h-full gap-1 justify-center mt-2">
              <button onclick="toggleOutputWindow()" class="w-1/2 border-2 border-indigo-500 hover:bg-indigo-500 text-white p-2 rounded-full flex justify-center items-center">Close</button>
            </div>
        </div>
      </div>
      <!-- Wildcards -->
      <div id="wildcards-window" class="fixed hidden z-30 top-0 right-0 w-full h-full flex justify-center items-center">
        <div class="w-[75%] max-w-md h-auto flex flex-col gap-2 p-4 bg-gray-800 border border-gray-700 rounded-lg">
            <div class="flex w-full h-full gap-2 justify-between">
              ...
              <button onclick="registerDomain()" class="p-2 rounded-full bg-blue-600 hover:bg-blue-700 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6"><path fill-rule="evenodd" d="M16.72 7.72a.75.75 0 0 1 1.06 0l3.75 3.75a.75.75 0 0 1 0 1.06l-3.75 3.75a.75.75 0 1 1-1.06-1.06l2.47-2.47H3a.75.75 0 0 1 0-1.5h16.19l-2.47-2.47a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" ></path></svg>
              </button>
            </div>
            <div id="container-domains" class="w-full h-48 rounded-md flex flex-col gap-1 overflow-y-scroll scrollbar-hide p-2 bg-gray-900"></div>
        </div>
      </div>
    </div>
    ...
    <footer>
      <div class="fixed bottom-3 right-3 flex flex-col gap-2 z-50">
        <a href="${DONATE_LINK}" target="_blank">
          <button class="bg-green-500 hover:bg-green-600 rounded-full border-2 border-gray-900 p-2 block">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6"><path d="M10.464 8.746c.227-.18.497-.311.786-.394v2.795a2.252 2.252 0 0 1-.786-.393c-.394-.313-.546-.681-.546-1.004 0-.323.152-.691.546-1.004ZM12.75 15.662v-2.824c.347.085.664.228.921.421.427.32.579.686.579.991 0 .305-.152.671-.579.991a2.534 2.534 0 0 1-.921.42Z" /><path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 6a.75.75 0 0 0-1.5 0v.816a3.836 3.836 0 0 0-1.72.756c-.712.566-1.112 1.35-1.112 2.178 0 .829.4 1.612 1.113 2.178.502.4 1.102.647 1.719.756v2.978a2.536 2.536 0 0 1-.921-.421l-.879-.66a.75.75 0 0 0-.9 1.2l.879.66c.533.4 1.169.645 1.821.75V18a.75.75 0 0 0 1.5 0v-.81a4.124 4.124 0 0 0 1.821-.749c.745-.559 1.179-1.344 1.179-2.191 0-.847-.434-1.632-1.179-2.191a4.122 4.122 0 0 0-1.821-.75V8.354c.29.082.559.213.786.393l.415.33a.75.75 0 0 0 .933-1.175l-.415-.33a3.836 3.836 0 0 0-1.719-.755V6Z" clip-rule="evenodd" /></svg>
          </button>
        </a>
        <button onclick="toggleWildcardsWindow()" class="bg-indigo-500 hover:bg-indigo-600 rounded-full border-2 border-gray-900 p-2 PLACEHOLDER_API_READY">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M9 9V4.5M9 9H4.5M9 9 3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5 5.25 5.25" /></svg>
        </button>
        <button onclick="toggleDarkMode()" class="bg-amber-500 hover:bg-amber-600 rounded-full border-2 border-gray-900 p-2">
    ...
    <script>
        ...
        const res = fetch(url).then(async (res) => {
          ...
          if (res.status == 200) {
            ...
            for (const domain of respJson) {
              ...
              domainElement.classList.add("w-full", "bg-gray-700", "p-2", "rounded-md");
      ...
      function applyRandomHeaderColors() {
        ...
        elements.forEach(el => {
            if (el) {
                ...
                el.className = el.className.replace(/text-\w+-\d+/g, '').replace(/dark:text-\w+-\d+/g, '');
                
                const randomColor = colors[Math.floor(Math.random() * colors.length)];
                // Tambahkan kelas baru
                el.classList.add(...randomColor.split(' '));
            }
        });
      }
          ...
          new Promise(async (resolve) => {
            const res = await fetch("https://${serviceName}.${rootDomain}/check?target=" + target)
              .then(async (res) => {
                if (isActive) return;
                if (res.status == 200) {
                  pingElement.classList.remove("text-gray-400", "dark:text-gray-500");
                  const jsonResp = await res.json();
                  if (jsonResp.proxyip === true) {
                    isActive = true;
                    pingElement.textContent = "Active " + jsonResp.delay + " ms " + "(" + jsonResp.colo + ")";
                    pingElement.classList.add("text-green-500", "dark:text-green-400");
                  } else {
      ...
      function checkGeoip() {
        const containerIP = document.getElementById("ip-value");
        const containerISP = document.getElementById("isp-value");
...
class Document {
    ...
    buildProxyGroup() {
        ...
        for (let i = 0; i < this.proxies.length; i++) {
            ...
            const target = `${proxyData.proxyIP}:${proxyData.proxyPort}`;

            proxyGroupElement += `<div class="lozad opacity-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-opacity duration-300 rounded-lg p-3 flex flex-col gap-2">`;
            // Header Kartu: Ping dan Bendera
            proxyGroupElement += `  <div class="flex justify-between items-center">`;
            proxyGroupElement += `    <div id="ping-${i}" data-target="${target}" class="text-xs text-gray-500 dark:text-gray-400">Idle</div>`;
            proxyGroupElement += `    <img width=24 src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" alt="${proxyData.country} flag" />`;
            proxyGroupElement += `  </div>`;
            // Nama ISP/ORG
            proxyGroupElement += `  <h3 class="font-bold text-teal-600 dark:text-teal-400 text-lg -mt-1 truncate">${proxyData.org}</h3>`;
            // Info IP dan Port
            proxyGroupElement += `  <div class="text-gray-600 dark:text-gray-300 text-sm -mt-2">`;
            proxyGroupElement += `    <p>IP: ${proxyData.proxyIP}</p>`;
            proxyGroupElement += `    <p>Port: ${proxyData.proxyPort}</p>`;
            proxyGroupElement += `  </div>`;
            // Tombol Konfigurasi
            proxyGroupElement += `  <div class="grid grid-cols-2 gap-2 mt-1 text-sm">`;
            ...
            for (let x = 0; x < proxyData.list.length; x++) {
                ...
                proxyGroupElement += `<button class="bg-blue-600 hover:bg-blue-700 rounded p-2 w-full text-white" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
            }
    ...
    addPageButton(text, link, isDisabled) {
        const pageButton = `<li><button ${isDisabled ? "disabled" : ""
            } class="px-4 py-2 bg-amber-500 text-gray-900 font-semibold rounded-lg hover:bg-amber-600 disabled:bg-gray-600 disabled:cursor-not-allowed" onclick=navigateTo('${link}')>${text}</button></li>`;
